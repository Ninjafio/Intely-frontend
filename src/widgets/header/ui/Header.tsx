		...
		<header className="header">
			...
			<div className="header__mobile-bar">
				...
				<div className="header__mobile-controls">
					<Cart />
					<button type="button" className="header__mobile-toggle" onClick={() => setIsMobileMenuOpen(true)} aria-label="Открыть меню">
			...
			{/* Mobile Overlay */}
			{isMobileMenuOpen && (
				<button type="button" aria-label="Закрыть меню" className="header__mobile-overlay" onClick={() => setIsMobileMenuOpen(false)} />
			)}
			...
			<div className={`header__mobile-drawer ${isMobileMenuOpen ? 'is-open' : ''}`}>
				<div className="header__drawer-top">
					...
					<button type="button" className="header__drawer-close" onClick={() => setIsMobileMenuOpen(false)} aria-label="Закрыть меню">
				...
				<div className="header__drawer-content">
					...
					<Link to={APP_PATHS.CATALOG} className="header__catalog-btn" onClick={() => setIsMobileMenuOpen(false)}>
						<Menu className="header__icon" />
						Каталог
					</Link>

					<button
						className="w-full px-3 py-2 rounded-lg bg-[#fff] border border-gray-300 text-black text-sm mb-2"
						onClick={() => (window.location.href = '/profile')}
					>
						Личный кабинет
					</button>
					{/*<button className="w-full px-3 py-2 rounded-lg border text-sm" onClick={logout}>
						Выйти
					</button>*/}

					<nav className="header__mobile-nav">
						{linkMap.map((link) => (
							<Link key={link.href} to={link.href} className="header__mobile-nav-link" onClick={() => setIsMobileMenuOpen(false)}>
								{link.text}
							</Link>
						))}
					</nav>

					<Link to="#" className="header__mobile-phone" onClick={() => setIsMobileMenuOpen(false)}>
						{PHONE_NUMBER}
					</Link>
				</div>
			</div>
